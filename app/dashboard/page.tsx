...
export default function DashboardPage() {
  ...
  const { myTasks, dueTodayTasks, overdueTasks, doneTasks } = useMemo(() => {
    ...
    const dueTodayTasks = userTasks.filter((t) => t.dueDate === today && t.status !== "done");
    const myTasks = userTasks.filter((t) => t.status !== "done");
    const overdueTasks = userTasks.filter((t) => isOverdue(t.dueDate) && t.status !== "done");
  ...
  const stats = [
    { label: "My Tasks", value: myTasks.length, icon: CheckCircle2, color: "text-brand-500", bg: "bg-brand-50 theme-dark:bg-brand-900/30 border-brand-100 theme-dark:border-brand-800/50" },
    { label: "Due Today", value: dueTodayTasks.length, icon: Calendar, color: "text-amber-500", bg: "bg-amber-50 theme-dark:bg-amber-900/30 border-amber-100 theme-dark:border-amber-800/50" },
    { label: "Overdue", value: overdueTasks.length, icon: AlertCircle, color: "text-red-500", bg: "bg-red-50 theme-dark:bg-red-900/30 border-red-100 theme-dark:border-red-800/50" },
    { label: "Completed", value: doneTasks.length, icon: TrendingUp, color: "text-emerald-500", bg: "bg-emerald-50 theme-dark:bg-emerald-900/30 border-emerald-100 theme-dark:border-emerald-800/50" },
  ];
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            {stats.map(({ label, value, icon: Icon, color, bg }, index) => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm font-medium text-slate-600 theme-dark:text-slate-300">{label}</span>
                  <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center bg-white/70 theme-dark:bg-slate-700/70")}> 
                    <Icon className={cn("w-4 h-4", color)} />
                  </div>
                </div>
                <p className="font-display font-bold text-3xl text-slate-800 theme-dark:text-slate-100">{value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* My tasks */}
            <div className="md:col-span-2 bg-white/80 theme-dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-white/70 theme-dark:border-slate-700/70 shadow-card overflow-hidden animate-fade-up" style={{ animationDelay: "160ms" }}>
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/70 theme-dark:border-slate-700/70">
                <h2 className="font-display font-semibold text-slate-800 theme-dark:text-slate-100">Assigned to me</h2>
                <span className="text-xs text-slate-400 theme-dark:text-slate-500">{myTasks.length} tasks</span>
              </div>
              <div className="divide-y divide-white/70 theme-dark:divide-slate-700/70">
                  ...
                  return (
                    ...
                    >
                      ...
                      {task.dueDate && (
                        <span className={cn(
                          "text-xs shrink-0",
                          overdue ? "text-red-500 font-semibold" : "text-slate-400 theme-dark:text-slate-500"
                        )}>
                ...
                {myTasks.length === 0 && (
                  <div className="px-5 py-8 text-center">
                    ...
                    <p className="text-sm text-slate-400 theme-dark:text-slate-500">All caught up! No tasks assigned.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Activity */}
            <div className="bg-white/80 theme-dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-white/70 theme-dark:border-slate-700/70 shadow-card overflow-hidden animate-fade-up" style={{ animationDelay: "220ms" }}>
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/70 theme-dark:border-slate-700/70">
                <h2 className="font-display font-semibold text-slate-800 theme-dark:text-slate-100">Activity</h2>
                  ...
                  <div key={item.id} className="flex gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-slate-600 theme-dark:text-slate-300 leading-snug">
                        ...
                        {" "}{item.action}{" "}
                        <span className="text-brand-600 theme-dark:text-brand-400 font-medium truncate">{item.target}</span>
                      </p>
                      <p className="text-[11px] text-slate-400 theme-dark:text-slate-500 mt-0.5">{timeAgo(item.createdAt)}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {dueTodayTasks.length > 0 && (
            <div className="bg-white/75 theme-dark:bg-slate-800/75 backdrop-blur-xl rounded-2xl border border-white/70 theme-dark:border-slate-700/70 p-5 shadow-card animate-fade-up" style={{ animationDelay: "280ms" }}>
              <div className="flex items-center gap-2 mb-4">
                <Clock className="w-4 h-4 text-amber-500" />
                <h2 className="font-display font-semibold text-slate-800 theme-dark:text-slate-100">Due Today</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                {dueTodayTasks.map((task) => (
                  ...
                  >
                    ...
                    <p className="flex-1 text-sm font-medium text-slate-700 theme-dark:text-slate-200 truncate">{task.title}</p>
